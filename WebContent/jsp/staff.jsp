...
<div class="x_title row">
	<h2 style="margin-bottom:10px;text-align: center;">员工信息列表 </h2>
	     <div class="clearfix"></div>
	</div>
	<hr>
 <div  style="margin-top:10px;" >
		 <div  class="layui-form-item" style="margin-top: 10px">
			 <label class="layui-form-label" style="color: black;">员工名称</label>
			 <div  class="layui-input-inline">
			 <input type="text" id="staffid" name="te1" placeholder="请输入员工名称"  class="layui-input">
			 	</div>
			 <div style="float: left">
			 <button id="btn1" class="layui-btn layui-btn-normal layui-btn-sm" onclick="selectstaff()">查询员工</button>
			 	</div>		   
			 <div>
			 <button id="btn2" class="layui-btn layui-btn-normal layui-btn-sm" style="float: right;margin-right: 10px" onclick="addshop()" >新增员工</button>
	 			</div>
		</div>				 
	 </div> 
...
<table class="layui-table">
		...
		<c:forEach items="${staffsBypage}" var="staff">
		  <c:set var="allPrice"    value="${car.carMessage.shopp_number *car.shopMessage.price }"></c:set>
			...
			<tr>
				...
				<td>${staff.staffpassword}</td>
                 <td>
                 <input type="button" class="layui-btn layui-btn-sm" value="修改" onclick="update(${staff.staffid})">
                   <input type="button" class="layui-btn layui-btn-sm" value="删除"  onclick="del(${staff.staffid})">
...
<div style="width: auto;margin-left: 500px;display: inline-block;">
   ...
   <c:forEach begin="1"   end="${allpage}"  var="page">
        <a href="${path}/staffShow?nowpage=${page}">
...
<form class="layui-form" action="${path}/addStaff" id="form1" hidden="hidden" style="margin-top: 20px;text-align:center;width: 400px;">
...
function selectstaff() {//查询信息
	...
	if (staffid=="") {
		...
		//alert("查询")
		window.location.href="${path}/operationStaff?operation=5&staffname="+staffid;
		//alert("已经查询")
	}
